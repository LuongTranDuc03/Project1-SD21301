    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        if (code && code.data) {
          ...
          reject(
            new Error(
              "Khong tim thay ma QR hop le trong anh. Vui long crop (cat) anh chi de lai ma QR, hoac chup gan, ro net nhat co the.",
            ),
          );
        }
      };
      img.onerror = () =>
        reject(
          new Error(
            "Loi khi doc file anh. Dam bao file tai len la dinh dang anh.",
          ),
        );
...
function parseCccdString(qrText) {
  ...
  if (rawDob && rawDob.length === 8) {
    dob =
      rawDob.substring(4, 8) +
      "-" +
      rawDob.substring(2, 4) +
      "-" +
      rawDob.substring(0, 2);
  }
