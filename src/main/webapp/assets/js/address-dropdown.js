...
function makeSearchableDropdown(target) {
  ...
  toggle.style.cssText =
    "display: flex; align-items: center; justify-content: space-between; cursor: pointer; background: #ffffff; padding: 10px 14px; border: 1px solid #cbd5e1; border-radius: 8px; font-size: 13px; color: #1e293b; user-select: none; width: 100%; min-width: 0; box-sizing: border-box; min-height: 40px;";

  const textSpan = document.createElement("span");
  textSpan.className = "custom-select-text";
  textSpan.style.cssText =
    "white-space: nowrap; overflow: hidden; text-overflow: ellipsis; flex: 1; min-width: 0; text-align: left;";

  const arrow = document.createElement("i");
  arrow.className = "fa-solid fa-chevron-down";
  arrow.style.cssText =
    "font-size: 11px; color: #64748b; margin-left: 8px; transition: transform 0.2s ease; flex-shrink: 0;";
  ...
  menu.style.cssText =
    "display: none; position: absolute; top: calc(100% + 4px); left: 0; right: 0; width: 100%; background: #ffffff; border: 1px solid #cbd5e1; border-radius: 8px; box-shadow: 0 10px 25px -5px rgba(0,0,0,0.15), 0 8px 10px -6px rgba(0,0,0,0.1); z-index: 999999; padding: 6px 0; max-height: 240px; flex-direction: column; box-sizing: border-box;";

  // Ô gõ tìm kiếm (Unicode Escapes)
  const searchBox = document.createElement("div");
  searchBox.style.cssText =
    "padding: 6px 8px 8px 8px; border-bottom: 1px solid #f1f5f9; box-sizing: border-box;";
  const searchInput = document.createElement("input");
  searchInput.type = "text";
  searchInput.placeholder = "G\u00f5 \u0111\u1ec3 t\u00ecm ki\u1ebfm..."; // Gõ để tìm kiếm...
  searchInput.style.cssText =
    "width: 100%; padding: 7px 10px; border: 1px solid #cbd5e1; border-radius: 6px; font-size: 12px; outline: none; box-sizing: border-box;";
  ...
  optionsBox.style.cssText =
    "max-height: 180px; overflow-y: auto; display: flex; flex-direction: column; width: 100%; box-sizing: border-box;";
  ...
  function renderOptions() {
    ...
    Array.from(select.options).forEach((opt, idx) => {
      if (opt.text.toLowerCase().includes(filterText)) {
        ...
        item.style.cssText =
          "padding: 8px 12px; font-size: 13px; line-height: 1.4; color: #334155; cursor: pointer; transition: background 0.15s; text-align: left; white-space: normal; word-break: break-word; border-bottom: 1px solid #f8fafc; box-sizing: border-box;";
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .querySelectorAll(".other-province, .other-district, .other-ward")
    .forEach((el) => {
