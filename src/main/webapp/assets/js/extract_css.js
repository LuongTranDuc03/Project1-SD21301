...
const assetsCssDir = path.join(__dirname, 'src', 'main', 'webapp', 'assets', 'css');
    ...
    for (const file of files) {
        ...
        if (stat.isDirectory()) {
            ...
            if (matches.length > 0) {
                ...
                const linkTagToRegex = new RegExp(`<link rel="stylesheet" href="\\\$\\{pageContext\\.request\\.contextPath\\}/assets/css/${targetFolder}/${cssFileName}">\\s*<link rel="stylesheet" href="\\\$\\{pageContext\\.request\\.contextPath\\}/assets/css/${targetFolder}/${cssFileName}">`, 'g');
                while (linkTagToRegex.test(content)) {
                    content = content.replace(linkTagToRegex, `<link rel="stylesheet" href="\${pageContext.request.contextPath}/assets/css/${targetFolder}/${cssFileName}">`);
                }
                
                if (hasChanges) {
                    fs.writeFileSync(fullPath, content, 'utf8');
                    console.log(`  -> Extracted CSS to assets/css/${targetFolder}/${cssFileName}`);
                }
            }
        }
    }
