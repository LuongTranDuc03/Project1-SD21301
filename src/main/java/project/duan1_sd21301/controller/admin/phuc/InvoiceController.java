...
public class InvoiceController extends HttpServlet {
    ...
    private void handleList(HttpServletRequest request, HttpServletResponse response)
        ...
        if (ttParam != null && !ttParam.isEmpty()) {
            try {
                orderStatus = Integer.parseInt(ttParam);
        ...
        if (pmiParam != null && !pmiParam.isEmpty()) {
            try {
                paymentMethodId = Integer.parseInt(pmiParam);
            } catch (NumberFormatException ignored) {
            }
        }
        ...
        if (otParam != null && !otParam.isEmpty()) {
            try {
                orderType = Integer.parseInt(otParam);
            } catch (NumberFormatException ignored) {
            }
        }
        ...
        List<Invoice> invoices = invoiceRepo.findAll(orderType, orderStatus, keyword, fromDate, toDate, paymentMethodId,
                page, PAGE_SIZE);
    ...
    private void handleExportExcel(HttpServletRequest request, HttpServletResponse response)
        ...
        List<Invoice> all = invoiceRepo.findAll(orderType, orderStatus, keyword, fromDate, toDate, paymentMethodId, 0,
                Integer.MAX_VALUE);
        ...
        try (XSSFWorkbook wb = new XSSFWorkbook()) {
            ...
            subCell.setCellValue(
                    "Xuat ngay: " + LocalDateTime.now().format(DateTimeFormatter.ofPattern("dd/MM/yyyy HH:mm")));
            ...
            for (Invoice inv : all) {
                ...
                c3.setCellValue(inv.getCustomerPhone() != null ? inv.getCustomerPhone() : "");
                c3.setCellStyle(dataStyle);
                Cell c4 = row.createCell(4);
                c4.setCellValue(inv.getCustomerEmail() != null ? inv.getCustomerEmail() : "");
                c4.setCellStyle(dataStyle);

                Cell c5 = row.createCell(5);
                if (inv.getTotalAmount() != null) {
                    c5.setCellValue(inv.getTotalAmount());
                ...
                if (inv.getPaidAmount() != null) {
                    c6.setCellValue(inv.getPaidAmount());
