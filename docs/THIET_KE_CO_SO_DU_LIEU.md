...
> **Tiêu chuẩn:** Định dạng bảng thiết kế theo báo cáo đồ án FPT Polytechnic  

---

## 📋 Danh Sách Bảng Trong Hệ Thống

| STT | Tên Bảng | Tên Tiếng Việt / Nghiệp Vụ | Phân Loại |
| :---: | :--- | :--- | :--- |
...
| STT | Tên Cột | Kiểu Dữ Liệu | Diễn Giải / Mô Tả | Nullable | Ràng Buộc |
| :---: | :--- | :--- | :--- | :---: | :--- |
| 1 | `id` | INT | Mã ID tự tăng | NOT NULL | PRIMARY KEY, IDENTITY(1,1) |
...
| 3 | `id_vai_tro` | INT | Khóa ngoại nối bảng vai_tro | NULL | FK -> `vai_tro(id)` |
| 4 | `id_dia_chi` | INT | Khóa ngoại nối bảng dia_chi | NULL | FK -> `dia_chi(id)` |
...
| 8 | `mat_khau` | VARCHAR(255) | Mật khẩu tài khoản (lưu BCrypt hash) | NOT NULL | |
...
| 5 | `mat_khau` | VARCHAR(255) | Mật khẩu (lưu BCrypt hash) | NOT NULL | |
...
| :---: | :--- | :--- | :--- | :---: | :--- |
| 1 | `id` | INT | Mã ID tự tăng | NOT NULL | PRIMARY KEY, IDENTITY(1,1) |
| 2 | `id_khach_hang` | INT | Khóa ngoại nối bảng khach_hang | NOT NULL | FK -> `khach_hang(id)` ON DELETE CASCADE |
| 3 | `id_dia_chi` | INT | Khóa ngoại nối bảng dia_chi | NOT NULL | FK -> `dia_chi(id)` ON DELETE CASCADE |
| 4 | `nguoi_nhan` | NVARCHAR(150) | Tên người nhận hàng | NULL | |
| 5 | `so_dien_thoai` | NVARCHAR(20) | Số điện thoại người nhận | NULL | |
| 6 | `mac_dinh` | BIT | Đánh dấu địa chỉ mặc định (1: Mặc định, 0: Không) | NOT NULL | DEFAULT 0 |
...
| 3 | `ten_danh_muc` | NVARCHAR(100) | Tên danh mục (Áo khoác da, Áo bomber,...) | NOT NULL | |
...
| 3 | `ten_xuat_xu` | NVARCHAR(100) | Tên quốc gia/xuất xứ (Việt Nam, Nhật Bản,...) | NOT NULL | |
| 4 | `trang_thai` | INT | Trạng thái (1: Hoạt động, 0: Ngừng) | NOT NULL | DEFAULT 1 |
| 5 | `created_at` | DATETIME | Thời điểm tạo | NOT NULL | DEFAULT GETDATE() |
...
| 4 | `id_danh_muc` | INT | Khóa ngoại danh mục sản phẩm | NULL | FK -> `danh_muc(id)` |
| 5 | `id_thuong_hieu` | INT | Khóa ngoại thương hiệu | NULL | FK -> `thuong_hieu(id)` |
| 6 | `id_xuat_xu` | INT | Khóa ngoại xuất xứ | NULL | FK -> `xuat_xu(id)` |
...
| 3 | `id_san_pham` | INT | Khóa ngoại sản phẩm cha | NOT NULL | FK -> `san_pham(id)` ON DELETE CASCADE |
| 4 | `id_kich_thuoc` | INT | Khóa ngoại kích thước | NULL | FK -> `kich_thuoc(id)` |
| 5 | `id_mau_sac` | INT | Khóa ngoại màu sắc | NULL | FK -> `mau_sac(id)` |
| 6 | `id_kieu_dang` | INT | Khóa ngoại kiểu dáng | NULL | FK -> `kieu_dang(id)` |
...
| * | *Composite Unique* | Constraint | Tổ hợp duy nhất (sp, size, màu, dáng) | - | `UQ_ctsp_variant` |
...
| 3 | `id_chi_tiet_san_pham` | INT | Khóa ngoại biến thể sản phẩm | NOT NULL | FK -> `chi_tiet_san_pham(id)` ON DELETE CASCADE |
...
| 4 | `loai_giam` | INT | Loai giảm giá (0: Giảm %, 1: Giảm VND) | NOT NULL | |
| 5 | `gia_tri_giam` | DECIMAL(18,2) | Giá trị giảm (% hoặc số tiền VND) | NOT NULL | |
| 6 | `gia_tri_don_hang_toi_thieu` | DECIMAL(18,2) | Điều kiện giá trị đơn hàng tối thiểu | NULL | |
| 7 | `giam_toi_da` | DECIMAL(18,2) | Số tiền giảm tối đa (với giảm %) | NULL | |
| 8 | `so_luong` | INT | Tổng phát hành | NULL | |
...
| 13 | `mo_ta` | NVARCHAR(MAX) | Mô tả chi tiết thể lệ chương trình | NULL | |
...
| 3 | `id_khach_hang` | INT | Khóa ngoại khách hàng mua | NULL | FK -> `khach_hang(id)` |
| 4 | `id_nhan_vien` | INT | Khóa ngoại nhân viên lập đơn | NULL | FK -> `nhan_vien(id)` |
| 5 | `id_dia_chi` | INT | Khóa ngoại địa chỉ giao | NULL | FK -> `dia_chi(id)` |
| 6 | `id_phuong_thuc_thanh_toan` | INT | Khóa ngoại phương thức TT | NULL | FK -> `phuong_thuc_thanh_toan(id)` |
| 7 | `id_ma_giam_gia` | INT | Khóa ngoại mã giảm giá | NULL | FK -> `phieu_giam_gia(id)` |
...
| 3 | `id_hoa_don` | INT | Khóa ngoại hóa đơn | NOT NULL | FK -> `hoa_don(id)` |
| 4 | `id_chi_tiet_san_pham` | INT | Khóa ngoại biến thể sản phẩm | NULL | FK -> `chi_tiet_san_pham(id)` |
...
| 3 | `id_hoa_don` | INT | Khóa ngoại hóa đơn | NOT NULL | FK -> `hoa_don(id)` |
| 4 | `id_nguoi_thuc_hien` | INT | Nhân viên thực hiện thay đổi | NULL | FK -> `nhan_vien(id)` |
| 5 | `id_khach_hang` | INT | Khách hàng thực hiện (nếu có) | NULL | FK -> `khach_hang(id)` |
...
## ⚡ Chỉ Mục Tối Ưu Hiệu Năng (Database Indexes)

Các chỉ mục (Index) đã được thiết lập sẵn trên các Khóa ngoại (FK) và cột tìm kiếm thường xuyên để tăng tốc độ JOIN & FILTER:
